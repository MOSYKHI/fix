...
void initiateGame(char* filename, Map* outMap, Game* outGame, Pacman* outPacman, Ghost* outGhosts) {
    ...
    FILE *my_file=fopen("/home/mosy/Desktop/96106088/pacman/cmake-build-debug/res/map.txt","a+");//set the file address

    fscanf(my_file,"%d%d\n",&outMap->width,&outMap->height);//scan width and height of map
    ///////////////
    ////////////
    ///////
    //scanning the map
    for (int i = 0; i < outMap->height ; ++i) {
        for (int j = 0; j < outMap->width; ++j) {
            fscanf(my_file,"%d",&outMap->cells[j][i]);
...
//set ghosts information
    for (int k = 0; k <MAX_GHOST_COUNT; ++k) {
        fscanf(my_file,"%d %d ",&outGhosts[k].dir,&outGhosts[k].blue);
...
void checkEatables(Map* map, Game* outGame, Pacman* outPacman, Ghost* outGhosts) {
    ...
    if (outPacman->x-(int)outPacman->x>=0.25&&outPacman->y-(int)outPacman->y>=0.25)
    {
